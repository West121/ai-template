...
export default function SendPage() {
  ...
  // 服务端分页 + 服务端搜索（keyword 防抖；后端 /api/office/doc/list 支持 pageNum/pageSize/keyword；
  // offline/未就绪时 fetchDocPage 内 mock 过滤+切片，分页语义一致）
  const query = useDebounced(keyword.trim())
  const page = useServerPage<GwDoc>(
    async (pageNum, pageSize) => {
      const res = await fetchDocPage({ ...toListQuery("SEND", filter), keyword: query || undefined, pageNum, pageSize })
      setDemo(res.demo)
      return res.data
    },
