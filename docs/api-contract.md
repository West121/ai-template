...
- **报表目录**（§11，批C，替换 stats_report）：GET `/reports?keyword=`、GET `/reports/{code}`（parameterSchema=参数白名单）；POST `/reports/{code}/execute` body `{params:{...}}`（白名单外参数 400；含 drill 参数（describe.drillParam）→ 明细 {columns,rows(≤20),total,datasetId?}，否则聚合 {chartType,categories,data,drill:{reportCode,paramName}}——前端图表下钻（亮点③）点击类目回调本端点）。种子：APPROVAL_COUNT_BY_STATUS/BY_TYPE/BY_PROCESS、DOCUMENT_COUNT_BY_TYPE、ATTENDANCE_RATE_BY_MONTH。
