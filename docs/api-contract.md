...
- GET `/api/system/users?keyword=&deptId=&enabled=&includeSubDept=&pageNum=` → {id,username,name,empNo,phone,email,gender:MALE|FEMALE|UNKNOWN,birthday,hireDate,officeLocation,leaderId,leaderName,avatar,remark,enabled,createdAt,primaryDeptName,primaryPostName,roleNames[]}；keyword 匹配 name/username/empNo/phone；deptId 按部门过滤，**includeSubDept**（默认 **true**）控制是否含子部门：true=含该部门 + 全部后代部门的用户（去重，复用 DEPT_AND_CHILD 的 ancestors 子树递归，点击父/公司部门可见其所有下级人员；部门闭包/物化路径为 DP1 性能优化项，本批用现有部门树递归），false=仅该部门直属任职用户；leaderName 由后端按 leaderId 一次 findAllById 组装（无 N+1）；GET `/{id}` 返回同样全字段 **+ `leaderIds:number[]`**（指定直属上级 id，按 sort_order 有序；列表接口不含，仅详情精确回填）
- POST `/api/system/users` {username,name,phone,password,deptId,postId,roleIds[],empNo?,email?,gender?,birthday?,hireDate?,officeLocation?,leaderId?,avatar?,remark?,**leaderIds?:number[]**}（建主任职；empNo 缺省自动生成 XC+4 位递增，传入则查重、唯一约束 uk_sys_user_emp_no；leaderIds 见下）【P:system:user:edit】
- PUT `/api/system/users/{id}` {name,phone,email?,gender?,birthday?,hireDate?,officeLocation?,leaderId?,avatar?,remark?,**leaderIds?:number[]**}（工号不可改；leaderId 不能为本人、须存在）；PUT `/{id}/enabled` {enabled}；POST `/{id}/reset-password`（B-12：重置为一次性随机初始密码，响应 `R<String>` data=新明文密码，供管理员转交用户；不再固定 admin123）；DELETE `/{id}`【P:system:user:edit】
  - **leaderIds（指定直属上级，多值有序 → `sys_user_leader`）**：`null`/不传=不改，`[]`=清空，非空=按数组顺序全量替换（写 sort_order）。防御：leader 必须是存在用户、**不能把自己设为自己上级**（后端拦，400）。供工作流 **LEADER（发起人主管）** 节点解析——发起人配了则用这组（节点 multiMode ANY/ALL 决定或签/会签），**没配则回退所在部门负责人 `dept.leader_id`**（向后兼容：未配用户行为不变）。与单值 `leaderId`（组织架构展示用）并存、互不影响。删除用户联带清理其 leaderIds 及以其为上级的悬挂引用。
...
### P3 高级能力（前缀 /api/wf）
运行时端点：
- POST `/api/wf/instances/{id}/predict` → `{path:[{nodeId,nodeName,type,assignees:[{name}], status,nodeType,canReject,rejectTo:{nodeId,name}?,multiMode,parallelGroup}], note?}` 流程预测**完整链路**（仅 DINGTALK）：从流程起点 DFS 走全程（**不跳过已完成节点**），按当前表单值/流程变量离线求值条件分支（结构化条件，白名单操作符），输出**已完成+当前+后续**整条链路 + 预计办理人（离线试算 ORG/LEADER/FORM_FIELD/INITIATOR），不落库。**新增字段**：`status`=done(已完成)/current(当前活动)/future(后续)（已结束实例全 done，note="流程已结束，展示完整链路"）；`nodeType`=节点类型(=type，兼容保留)；`canReject`=审批节点可驳回（流程级 operations.reject 未关 + 节点 allowedOps 未排除 reject，缺省 true）；`rejectTo`={nodeId,name} 驳回回退目标（flowConfig.rejectStrategy=PREV→上一审批节点，否则回发起人；拿不到发起人兜底）；`multiMode`=ALL/ANY/SEQUENCE/VOTE 并签模式（非审批 null）；`parallelGroup`=并行网关分组 id（同组前端并排，多路都纳入）。**向后兼容**：既有 nodeId/nodeName/type/assignees 不变，旧前端只读这四项不受影响。BPMN 专业模式返回空 path + note
...
## AI 智能助手（oa-boot ai 域，V28/V33-V35，前缀 `/api/ai`）
> 契约 `docs/design/ai-assistant-design-v2.md`。登录即用；工具经 AuthorizedToolResolver 权限过滤暴露 + AiToolGateway 执行复验（@AiToolDefinition authorities/risk/timeout）；主链路 Spring AI 2 ChatClient+Advisor 链（批B），SSE 异步执行、UserContext/SecurityContext 显式传播（批A）。
- **SSE**：POST `/chat/messages` {sessionId?, clientMessageId?, message, modelProfileId?|credentialId?, model?, attachments?, pageContext?{featureCode,entityType,entityId}} → text/event-stream，事件 `message.started → tool.started/completed|failed → message.part.created ×N → message.completed|failed`（事件体 {sessionId,messageId,sequence,type,timestamp,payload}，sequence 递增）。clientMessageId 幂等（unique(tenant,user,cmid)，重试重放原回复不调模型）；会话串行（RUNNING 时 409 AI_SESSION_BUSY）。pageContext 经服务端校验 featureCode 可见后注入系统提示。
- POST `/chat`（阻塞回退，形状兼容）{同上} → {sessionId, messages:[{role,content,cards?,parts?}]}。§11 多模态（IMAGE content parts base64 / TEXT 16k 引用块 / supportsVision 检测 400）不变。
- **Part 协议**（§9.3）：助手消息 Part 化落 ai_chat_message_part；partType=`text`{text, citations?[{sourceType:FEATURE|REPORT|DATA,sourceId,title}]（亮点④）} / `plan`{title,steps:[{title,status:pending|done}]}（亮点①，同 partId 重推=状态更新）/ 六类卡映射（card.type 即 partType，payload=卡片原文）。旧 cards 字段兼容期并存输出。
- **动作草稿**（§7，ai_action_draft 状态机）：POST `/actions/{id}/confirm`（空 body，`Idempotency-Key` 头幂等重放；过期/取消 410 AI_ACTION_EXPIRED、重复 409 AI_ACTION_ALREADY_EXECUTED、TOCTOU 409 AI_ACTION_STALE、他人 403）；POST `/actions/{id}/cancel`；旧 POST `/confirm` {actionId} 兼容代理（一律 410/403 旧语义）。
- **模型**：GET `/models`（V1 凭据列表，兼容）；GET `/model-profiles`（§4.3，[{id,code,name,description,available,supportsVision}]，凭据/密钥永不出 API）；chat 请求 modelProfileId（FAST/STANDARD/...）与 credentialId 并存（credentialId 优先，V1 兼容）。
- **功能目录**（§12.1，批C，种子自 menu.ts）：GET `/features`（当前用户可见，[{featureCode,moduleCode,name,description,routeCode,keywords}]）；GET `/features/{code}`（受控导航校验：非法 400 / 不可见 403）。featureCode 约定=path 段大写下划线（`/workflow/form-defs → WORKFLOW_FORM_DEFS`）。
- **报表目录**（§11，批C，替换 stats_report）：GET `/reports?keyword=`、GET `/reports/{code}`（describe 含 parameterSchema=参数白名单、drillParam，以及**受控灵活维度** allowedDimensions(`[{code,label}]`)/allowedTimeGrains(`[day,week,month,quarter,year]`)/defaultDimension）；POST `/reports/{code}/execute` body `{params:{...}}`（白名单外参数 400；含 drill 参数（describe.drillParam）→ 明细 {columns,rows(≤20),total,datasetId?}，否则聚合 {chartType,categories,data,drill:{reportCode,paramName[,dimension]}}——前端图表下钻（亮点③）点击类目回调本端点）。种子：DOCUMENT_COUNT_BY_TYPE、ATTENDANCE_RATE_BY_MONTH，及审批量多维数据源见下。
- **受控灵活维度**（§11.1 批E，解决报表「太死板」）：审批量合并为一个多维数据源 `APPROVAL_COUNT`（required_authorities=NULL，个人可见范围内自助统计；数据权限仍由 office `SecuritySupport.dataScope` 约束可见行）——`params.dimension` 从 allowedDimensions=`status/type/process/month/dept/initiator` 内选（`report_execute` 工具亦接受 dimension/timeGrain 便捷平铺；未在白名单 → 400 友好错误「报表[…]不支持维度[…]。支持的维度：状态(status)、…」，供 AI 改用合法维度重试），时间维加 `params.timeGrain`（默认 month）、可选 `params.rangeStart/rangeEnd`（yyyy-MM-dd 按创建时间过滤）、下钻用 `params.drillValue`。**安全红线不破**：dimension/timeGrain 仅是服务端白名单内的**枚举选择**，映射到**预定义**分组逻辑（oa_approval 走 JPA `Specification`+Java 分组；process 走 wf_instance_ext 参数化 native），LLM 传入的字符串**绝不拼进 SQL 字段名/片段**——不是隐形 SQL。chartType 按维度默认（month→line、status→pie、其余分类维→bar）。旧 3 个 reportCode **APPROVAL_COUNT_BY_STATUS/BY_TYPE/BY_PROCESS 保留为预设别名**（单维 allowedDimensions，向后兼容原聚合/下钻结果）。
- **数据集**（§11.2）：>20 行明细落 ai_dataset（30min 过期）；GET `/datasets/{id}?pageNum=&pageSize=` → `{rows, page:{current,size,total}, columns}`（归属再鉴权 403 / 过期 410）。
- GET `/sessions` 近 30 天会话列表（用户隔离）；GET `/sessions/{id}/messages?pageNum=&pageSize=`（缺省 1/50）→ PageResult{list:[{id,role,status,sequenceNo,content,cards?,attachments?,parts?(ASSISTANT),createdAt}],total,...}；DELETE `/sessions/{id}`（连带 parts；ai_tool_call 审计独立保留）。
- **Card 类型**：navigate{**featureCode,routeParams**,path(兼容),title,desc?} / list{title,columns,rows,moreLink?,total?,datasetId?} / confirm{actionId,title,summary,params,danger,expiresAt?} / form{defCode,defName,formType,**schema=widgets 数组**（不做对象包裹/JSON 字符串，契约以数组为准）,submitPath?(CODE),**prefill?={fieldKey:value}**（§8.1 对话式表单预填：后端从 LLM knownValues 提取并按 schema 清洗——仅留真实字段 key、select 值归一到选项 value；前端透传 FormRenderer initialValues；即 V2 part.payload.prefill，无预填时省略）} / chart{chartType,title,categories?,series,reportCode?,drill?,datasetId?} / link{items[{title,path,featureCode?}]} / error{code,message,missingAuthority?,holderRoles?,adminHint?（亮点② 权限解释）} / **devDiff**（批W2 Dev Studio 变更提案卡）{actionId,assetType(ORCH|PROCESS|FORM|BIZDOC_TPL),code,name,oldContent,newContent,summary,baseVersion(快照版本=TOCTOU 基线),publish,danger(=publish),effectNote(生效面文案)}——前端 buildLineDiff 渲 diff，确认走 POST /actions/{id}/confirm 复用二段式
- **工具目录**（§6.4 规范名，旧名别名兼容）：feature_get_user_capabilities/navigation_open（读 ai_feature_catalog 按权限过滤）；task_query_my_tasks/workflow_query_my_instances/document_query_mine/meeting_query/leave_query_balance/attendance_query_month/urgent_query_mine；report_search/report_describe/report_execute（别名 stats_report，旧 module/dimension 入参映射）；workflow_prepare_start(form 卡；入参 defCode + knownValues?——从用户话语提取的已知字段值，后端清洗为 card.prefill 预填)/task_prepare_approve/schedule_prepare_create/meeting_prepare_create/**memory_remember**(confirm 卡，CONFIRM_REQUIRED)；**dev_list_assets/dev_read_asset**（READ_ONLY，authorities=dev:studio:view——list 卡/内容+版本给模型，content>6k 截断标 contentTruncated）/**dev_propose_change**（CONFIRM_REQUIRED，dev:studio:edit——服务端读当前内容作 oldContent+当前快照版本作 baseVersion，propose 阶段先干跑校验 newContent+资产写码预检（坏内容/无权→error 卡不产草稿），stage 动作草稿产 devDiff 卡；确认执行=调 DevStudioService.save(actor=AI)，双门/干跑/乐观锁复用，快照冲突 409「资产已被修改…请重新发起」）（批W2）。
- **上下文与摘要**（批D，V36，§13）：短期上下文按 **token 预算**裁剪（估算 token≈字符数/2，`ai-assistant.context.token-budget` 默认 8000，超预算丢最早）；**结构化滚动摘要**（累计 token ≥ `ai-assistant.summary.token-threshold` 默认 8000 触发，压更早消息为 JSON{userGoal,activeEntities,resolvedReferences} 存 ai_chat_session.summary，游标 summarized_until_message_id 推进、注入 system）。ChatMemory 自实现读 ai_chat_message，不引 Spring AI Jdbc memory。
- **长期记忆**（批D，§13.4，ai_user_memory）：对话工具 memory_remember（CONFIRM_REQUIRED 二段式）落库偏好；相关时 keyword 检索注入 system（不上向量）。GET `/memories`（当前用户活跃记忆 [{id,memoryType,memoryKey,memoryValue,confidence,...}]）；DELETE `/memories/{id}`（软删，他人 403）。**敏感黑名单**（密码/token/身份证/银行卡/工资/薪酬）命中拒存（400，不生成确认卡）。
- **附件 fileId 化**（批D，§17）：POST `/attachments`（multipart file）→ 校验 MIME/大小（图片 png/jpg/webp≤5MB、文本 txt/md/csv/json/log≤1MB）→ {attachmentId,kind,name,url}；chat 请求 attachments 收 {attachmentId,kind,name}（dataUrl 兼容期仍收，入库前归一化：dataUrl 抽到 FileService、消息表只存 attachmentId 引用不存 base64）；多模态取文件转 base64 content part（supports_vision 档案才带图）；引用越权 403。POST `/attachments/clean-legacy`【system:dict:edit】存量 dataURL 清洗（幂等 {scanned,cleaned}，启动 runner 亦执行一次）。
- **RAG**（批D，§12.2，pgvector）：docker postgres 镜像 `pgvector/pgvector:pg17`；RetrievalAugmentationAdvisor 以最后 user 消息检索 ai_knowledge_doc（status/module_code 过滤），命中以「参考资料」包裹注入 system + text part citations 加 RAG_DOC{sourceType:RAG_DOC,sourceId,title}。**嵌入**：优先复用 LLM 凭据 /embeddings；DeepSeek 无 embeddings → 默认走 PostgreSQL 全文/ILIKE(2-gram)降级检索（`ai-assistant.rag.embedding-enabled` 默认 false，降级默认可用），有嵌入凭据则 `<=>` 向量近邻。知识 admin：GET/POST `/knowledge`、DELETE `/knowledge/{id}`【system:dict:edit】；种子 3 篇（请假制度/公文办理流程/系统使用指南）。
- **晨报**（批D，附3 亮点⑤）：GET `/briefing` → {date,greeting?,generatedAt,urgentCount(急事数),meetingCount(今日会议数),unreadCount(未读待阅数),items:[{title,kind:"待办"|"会议"|"待阅",featureCode(急事/待阅→WORKFLOW_TASKS、会议→MEETING_MY),routeParams,path?(兜底),meta?(原因/明细)}],cached}（items 合并急事 Top-5(复用服务端急事排序)+今日会议+未读待阅，每项带 featureCode+routeParams 供前端 route-registry 跳转；当日首次生成、当日缓存；编排定时接入留扩展点 AiBriefingService.generate）。
- **受控管理操作**（批M1，`docs/design/ai-managed-actions.md`，前缀 `/api/ai/manage`）：AI 放开常用增改（新增/编辑），走白名单预注册 + 权限继承 + 确认二段式。**可扩展**：业务模块实现 `oa-common` 的 `AiManagedActionProvider` bean 返回 `AiManagedAction` 描述符（actionCode/module/entityLabel/action(CREATE|UPDATE)/label/formSchema(widgets)/requiredAuthority/handler 绑定 Service 方法/updateLoader），启动被 `AiManagedActionRegistry` 收录 → 三通用工具与 REST 自动覆盖，**不改 AI 核心**（依赖方向：ai 侧发现描述符，业务模块不依赖 boot；批M1 的 system 描述符由 boot 侧 provider 集中声明，保持 system 零 AI 依赖）。
  - **3 通用能力**（替代逐业务写工具）：LLM 工具 `manage_list_actions(keyword?,module?)`（READ_ONLY，返回**当前用户 requiredAuthority⊆权限**的操作，红线：无权不暴露）+ `manage_prepare(actionCode,knownValues?,targetId?)`（READ_ONLY，产表单卡：formSchema+预填；UPDATE 按 targetId 经 updateLoader 载现值）；提交是 UI 动作走 REST（非 LLM）。
  - GET `/manage/actions?keyword=&module=` → `[{actionCode,module,entityLabel,action,label}]`（有权操作，前端提示面板/免 LLM 轮次）。
  - POST `/manage/prepare` {actionCode,knownValues?,targetId?} → **manage_form 卡** `{type:"manage_form",actionCode,action,module,entityLabel,title,schema=widgets 数组,prefill?,targetId?(UPDATE),submitPath:"/api/ai/manage/submit"}`（schema 与 workflow form 卡同构，仅提交端点不同；无权 403 / 未注册 404 / UPDATE 缺 targetId 400）。
  - POST `/manage/submit` {actionCode,values,targetId?} → **confirm 卡**（复用批A `{type:"confirm",actionId,title,summary,params:[{label,value}](密码脱敏),danger:false}`）：暂存动作草稿(ai_action_draft，tool_name=`managed_action`)；执行走 POST `/actions/{id}/confirm`（复用批A 二段式）→ `AiManagedGateway` **权限二次校验（继承 requiredAuthority）+ 值反序列化进绑定 DTO（字段限 schema）+ Bean Validation（补位控制器 @Valid）+ 反射调业务 Service（其事务/唯一性校验生效）**。无权 403 / 未注册 404 / 缺必填 400。
  - **批M1 已注册**（system 域，requiredAuthority=各自 `system:*:edit`）：system.user.create/update、dept.create/update、role.create/update、post.create/update。**删除/改权限/重置密码不注册**（高危）。审计：manage 工具落 ai_tool_call、动作执行落 ai_action_draft。
...
- **定义**：GET `/defs?keyword=&status=&pageNum=`【read】、GET `/defs/published`（运行时入口卡）、GET `/defs/{key}`（数字=id 否则=code）；POST/PUT/DELETE `/defs...`、POST `/defs/{id}/publish|disable`【def:write】。Def 字段含 **submitPath**（CODE 表单运行时「新建」跳转路径）与 **formSchema**（§10 单据私有表单 schema，{widgets:[...]} 与在线表单同构，不进 wf_form_def）。
- **form_type 语义（§10 范式修正二，V30）**：`INLINE`（内置字段设计，默认/主路径——录入 FormRenderer/台账列/打印字段树全从私有 formSchema 派生）| `CODE`（高级：绑手写表单+submitPath）。ONLINE 选项移除，存量 ONLINE 定义兼容读（按外部 wf 表单引用）。发布校验：INLINE=formSchema 非空+至少一个字段+key 全局唯一；CODE/存量 ONLINE=表单存在(wf_form_def)；编号规则存在/wf_def_code 已发布/list_config 字段 ∈ 对应字段清单。
- **流程字段识别联动（§10）**：`GET /api/wf/forms/bizdoc:{defCode}/fields` → `{formKey:"bizdoc:{code}", formType:"BIZDOC", fields:[FieldDescriptor]}`（从 def.formSchema 经 FormManifestExtractor 同源派生；流程设计器条件/取人由此识别单据字段）。冒号在 URL 路径段合法——原样或 %3A 编码均可路由。
...
- **打印**：GET `/defs/{defId}/print-tpls`、POST（首个自动默认）、PUT/DELETE `/print-tpls/{id}`、POST `/print-tpls/{id}/default`【def:write】；GET `/docs/{id}/print?tplId=`【read】→ `{tpl(content 元素树 §4), data(form_data+docNo/title/creatorName/deptName/createdAt/status——status=VOID 前端渲 45° 作废水印 §8, **_approvals**=办理记录数组 `[{nodeName,assigneeName,opinion,time}]`（§9.3 审批记录区：绑流程单据从其流程实例取，按办理顺序，与已办/时间线同源=wf_operation；无流程或未办=[]）；**选人类字段解析**（关联字段显示属性）：form_schema 中 type=user|dept 的字段，存储值(id/OrgRef/数组)解析为 `{id,name[,username]}` 对象（数组→对象数组），模板经点路径 `{{field.name}}`/`{{field.0.name}}` 取显示属性，另给**平铺便利键 `{field}_names`**（多值「、」join，如"张三、李四"）；解析失败保留原值不阻断打印), fields([{key,label}])}`——**渲染在前端**（套打设计器同一渲染器）。
- 占号：`DocNumberService.allocateExternal`（原子序号+台账行 document_id 置空，与公文幂等互不串号；单据幂等=已有 docNo 跳过）。
- **单据模板独立化（§11，V31，前缀 `/api/bizdoc/tpls`）**：模板独立于单据定义，`bindType=BIZDOC|FLOW|FORM`（BIZDOC=defId 附属原路径，存量回填；FLOW=wf defCode / FORM=formCode 走 bindCode），`code` 唯一（缺省自动生成 tpl_xxxxxxxx，不可改）、`status DRAFT|PUBLISHED`（存量回填 PUBLISHED）、`version` 发布自增（DRAFT 新建=0）。
  - CRUD：GET ``（分页 keyword=name/code like、category、bindType；列表项 content=null 省流量）、GET/PUT/DELETE `/{id}`、POST ``（仅 FLOW/FORM；校验 bindCode 存在：FLOW→wf_process_ext 已发布 / FORM→wf_form_def）、POST `/{id}/publish`（status→PUBLISHED，version+1）【管理 def:write；读 read】。
