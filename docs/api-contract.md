...
- **打印**：GET `/defs/{defId}/print-tpls`、POST（首个自动默认）、PUT/DELETE `/print-tpls/{id}`、POST `/print-tpls/{id}/default`【def:write】；GET `/docs/{id}/print?tplId=`【read】→ `{tpl(content 元素树 §4), data(form_data+docNo/title/creatorName/deptName/createdAt/status——status=VOID 前端渲 45° 作废水印 §8), fields([{key,label}])}`——**渲染在前端**（套打设计器同一渲染器）。
