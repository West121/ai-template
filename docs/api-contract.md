...
- POST `/chat` {sessionId?, message, credentialId?, model?, attachments?} → {sessionId, messages:[{role:"ASSISTANT", content:markdown, cards?:Card[]}]}（sessionId 空=新会话）。**§11 增强**：credentialId 覆盖默认凭据（校验存在/LLM 型/启用，否则 400）、model 再覆盖凭据 model；attachments=[{fileId|dataUrl, kind:IMAGE|TEXT, name}]——IMAGE→OpenAI content parts（image_url base64 dataURL；fileId 经 infra 读字节转 base64），TEXT→读文本截 16k 以引用块注入 user content；**能力检测**：带 IMAGE 且凭据 !supportsVision → 400「当前模型不支持图片，请切换支持视觉的模型」；上游多模态报错转译友好提示（200 assistant 文案）。附件 JSON 存 ai_chat_message.attachments（V32），messages 接口回显。历史轮次仅重放文本 content（附件不重发，控上下文体积）。
- GET `/models`（§11 模型切换）→ 启用的 LLM 凭据列表 [{id,name,model,supportsVision}]（orch_credential.supports_vision，V32；凭据 CRUD DTO 带 supportsVision）
