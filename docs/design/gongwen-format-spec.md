...
4. **文号六角括号**：`〔〕`（U+3014/U+3015），不是方括号 `[]`、不是全角方括号 `【】`。年份用四位全称：`星辰办〔2026〕15号`。
...
- **红色、居中**，小标宋体，字号由机关自定（一般一号或更大，联合行文可更大）。文字如"星辰科技有限公司文件""星辰科技有限公司办公室文件"。
- 上边缘（红头首字上沿）距版心上边缘推荐约 **35mm**（企业化预览可放宽，保持视觉留白即可）。
- "×××文件"整体作为一个红色标志块，居中；"文件"二字与机关名同字号同色。

```html
<div class="gw-header">{issuingOrg}</div>   <!-- 例：星辰科技有限公司文件 -->
```
...
- **印发机关和印发日期**同一行：印发机关左空一字，印发日期右空一字（加"印发"二字），如 `星辰科技有限公司办公室　　2026年7月10日印发`。
